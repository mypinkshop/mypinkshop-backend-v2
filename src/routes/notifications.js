...
// ✅ GET /api/notifications/admin/sent (Admin) - list every notification ever sent
// (Frontend AdminNotifications.jsx isko hit karta hai)
notifications.get('/admin/sent', authMiddleware, requireAdmin, async (c) => {
  try {
    const { results } = await c.env.DB.prepare(
    ...
    return fail(c, `Failed to load sent notifications: ${err.message}`, 500);
  }
});

// ✅ DELETE /api/notifications/admin/:id (Admin)
notifications.delete('/admin/:id', authMiddleware, requireAdmin, async (c) => {
  try {
    const id = c.req.param('id');
    const result = await c.env.DB.prepare('DELETE FROM notifications WHERE id = ?').bind(id).run();
