...
userCards.post('/', authMiddleware, async (c) => {
  try {
    ...
    const { cardNumber, cardHolderName, expiryMonth, expiryYear, cvv, isDefault } = body;
    
    if (!cardNumber || !cardHolderName || !expiryMonth || !expiryYear) {
      return fail(c, 'cardNumber, cardHolderName, expiryMonth, expiryYear are required.', 400);
    }
    
    // ✅ Kaam ke liye card number ko encrypt karna hoga (mast security ke liye)
    // Abhi ke liye simple store karte hain
    ...
    return ok(c, { id, last4, cardHolderName, expiryMonth, expiryYear, isDefault }, undefined, 201);
  } catch (err) {
