...
admin.get('/dashboard', authMiddleware, requireAdmin, async (c) => {
  try {
    // Users stats
    // ✅ FIX: schema's users.role CHECK constraint only allows
    // 'customer' | 'admin' | 'vendor' — there is no 'buyer' role, so this
    // count was always returning 0. Changed to 'customer'.
    const [totalUsers, totalVendors, totalBuyers, totalProducts, totalOrders] = await Promise.all([
      ...
      c.env.DB.prepare('SELECT COUNT(*) as count FROM users WHERE role = ?').bind('customer').first(),
